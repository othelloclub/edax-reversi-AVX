...
enum {
	ANALYZE_SOURCE_SEARCH = 0,
	ANALYZE_SOURCE_BOOK = 1
};

typedef void (*AnalyzeResultHook)(const AnalyzeHookRow *row);
...
void play_analyze_with_book(Play*, int);
extern AnalyzeResultHook g_analyze_result_hook;
int play_alternative(Play*, Move*, Move*, int*, int*);
int play_book_alternative(Play*, Move*, Move*);
