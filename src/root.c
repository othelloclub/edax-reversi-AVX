...
int PVS_root(Search *search, const int alpha, const int beta, const int depth)
{
	...
	if (movelist_is_empty(movelist)) {
		...
		if ((move = node_first_move(&node, movelist))) {
			...
			if (search->current_move_hook) search->current_move_hook(search, move);
			...
			if (search->current_move_result_hook) search->current_move_result_hook(search, move, depth, search->selectivity);
