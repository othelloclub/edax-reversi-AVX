...
typedef struct Search {
	...
	void (*current_move_hook)(struct Search*, Move*); /**< call back: about to search this root move */
	void (*current_move_result_hook)(struct Search*, Move*, int, int); /**< call back: root move scored at current depth/selectivity */
} Search;
...
/* ASYNCIFY yield: WASM 실행 중 주기적으로 JS event loop로 제어 양도.
 * bridge.c의 edax_yield_to_host()가 emscripten_sleep(0)을 호출한다.
 * search->n_nodes 마스킹으로 호출 빈도 제어 (65536 노드마다).
 * 비-Emscripten 빌드에서는 no-op. */
#ifdef __EMSCRIPTEN__
	extern void edax_yield_to_host(void);
	#define EDAX_ASYNC_YIELD_MASK 0x3FFFu
	#define EDAX_ASYNC_YIELD(search) \
		do { if (((search)->n_nodes & EDAX_ASYNC_YIELD_MASK) == 0) edax_yield_to_host(); } while (0)
#else
	#define EDAX_ASYNC_YIELD(search) ((void)0)
#endif
