...
AnalyzeResultHook g_analyze_result_hook = NULL;

static void play_emit_analyze_row(Play *play, int ply, const Move *played, int played_score, const Move *alternative, int legal_count, int depth, int percent, int source)
{
	AnalyzeHookRow row;

	if (!g_analyze_result_hook || legal_count < 0) return;
	...
	row.percent = percent;
	row.source = source;
	g_analyze_result_hook(&row);
}

static int play_book_first_alternative(Play *play, Move *played, Move *alternative, int *depth, int *percent, int *source)
{
	int n_alternatives = 0;

	*source = ANALYZE_SOURCE_SEARCH;
	if (options.book_allowed && play->book) {
		Move book_alternative;
		book_alternative.x = NOMOVE;
		n_alternatives = play_book_alternative(play, played, &book_alternative);
		if (n_alternatives >= 0) {
			*alternative = book_alternative;
			*depth = -1;
			*percent = 100;
			*source = ANALYZE_SOURCE_BOOK;
			return n_alternatives;
		}
	}

	n_alternatives = play_alternative(play, played, alternative, depth, percent);
	return n_alternatives;
}
 ...
 */
int play_alternative(Play *play, Move *played, Move *alternative, int *depth, int *percent)
{
	Search *search = &play->search;
...
void play_analyze(Play *play, int n)
{
	...
	for (i = play->i_game - 1; i >= 0 && i >= play->i_game - n; --i) {
		...
		play_emit_analyze_row(play, i, move, move->score, &alternative, n_alternatives, depth, percent, ANALYZE_SOURCE_SEARCH);
		fflush(stdout);
 ...
 * @return The number of alternatives.
 */
int play_book_alternative(Play *play, Move *played, Move* alternative)
{
	MoveList movelist;
	Move *exclude, *best;
	Board excluded, board, unique;
...
void play_book_analyze(Play *play, int n)
{
	...
	for (i = play->i_game - 1; i >= 0 && i >= play->i_game - n; --i) {
		...
		play_emit_analyze_row(play, i, move, move->score, &alternative, n_alternatives, -1, 100, ANALYZE_SOURCE_BOOK);
...
/**
 * @brief Analyze a game with book-first priority and result hook.
 *
 * For each ply (from last to first): checks book first, falls back to engine.
 * Emits the same played/best score semantics as play_analyze().
 *
 * @param play Play (must be positioned at the end of the game).
 * @param n number of moves to analyze.
 */
void play_analyze_with_book(Play *play, int n)
{
	int i, score, n_alternatives;
	int depth = 0, percent = 0;
	int source = ANALYZE_SOURCE_SEARCH;
	Move *move, alternative;
	...
	for (i = play->i_game - 1; i >= 0 && i >= play->i_game - n; --i) {
		...
		n_alternatives = play_book_first_alternative(play, move, &alternative, &depth, &percent, &source);
		play_write_analysis(play, move, &alternative, n_alternatives, depth, percent, stdout);
		play_emit_analyze_row(play, i, move, move->score, &alternative, n_alternatives, depth, percent, source);

		score = move->score;
		if (n_alternatives > 0 && alternative.score > score) score = alternative.score;

		if (play->search.stop == STOP_ON_DEMAND) break;
	}

	// restore board to end position
	if (i < 0 || i < play->i_game - n) ++i;
	for (; i < play->i_game; ++i) {
		board_update(&play->board, play->game + i);
		play->player ^= 1;
