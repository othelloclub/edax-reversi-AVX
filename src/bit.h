...
// bit_intrinsics: CPU dependent bit operation intrinsics.
	...
	// wasm 은 포인터가 32bit 라도 64bit 정수 연산이 네이티브다 → 비트보드는 64bit 경로 사용.
	// (이로써 32bit-x86 호환 shim 블록[!HAS_CPU_64]이 비활성화되어 emscripten compat 헤더의
	//  _mm_cvtsi64_si128/_mm_cvtsi128_si64/_mm_extract_epi64 와의 중복정의 충돌도 사라진다.)
	#define	HAS_CPU_64	1
#endif
	...
	// emscripten: catch-all <x86intrin.h> 는 wasm 미지원 AMD 헤더(ammintrin/SSE4a,
	// fma4intrin/FMA4)까지 끌어와 컴파일이 깨진다. SSE→SIMD128 에 필요한 것은 immintrin.h 로 충분.
	#include <immintrin.h>
  #else
	#include <x86intrin.h>
  #endif
...
#if (defined(__SSE4_2__) || defined(__AVX__)) && !defined(__EMSCRIPTEN__)
	// emscripten/wasm 에는 하드웨어 CRC32(_mm_crc32_*) 명령이 없다 → 아래 #else 의
	// 소프트웨어 테이블 CRC32c(bit.c, #ifndef crc32c_u64 로 자동 컴파일)로 폴백. (CRC 는 해시키용이라 핫루프 아님.)
  #ifdef HAS_CPU_64
...
#if (defined(_MSC_VER) || defined(__clang__)) && defined(hasSSE2) && !defined(__EMSCRIPTEN__)
